...
\title{Nothing, the Boundary and Everything}
...
Most mathematicians would immediately recognize that this axiom leads to paradoxes. If \textit{every} proposition has a witness, then contradictory propositions must also have witnesses. But sometimes, it's worthwhile to break things and see what happens. Omega represents a strange, alien realm in mathematics. Whereas we are accustomed to operating within the safe confines of consistency and certainty, Omega is a space where anything goes, including paradoxes. Maybe this is not a bug, but a feature: Omega's completeness means it must contain witnesses for \textit{every} possible proposition, even contradictory ones. Let's follow this thread and see where it takes us.
...
We have seen that absolute completeness (Omega) leads to triviality through paradox, while absolute emptiness (Nomega) leads to triviality through vacuity. Useful logic must lie somewhere between these extremes.
...
\text{NomegaType}: &\quad \forall x, False \quad \text{(there are no witnesses)}
\end{align}
