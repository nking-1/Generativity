...
in the paper have a formalized counterpart in Rocq. References to the relevant Rocq proofs are provided throughout the text.
...
\begin{itemize}
    ...
    \item Category theory fundamentals (e.g., categories, adjunctions, monads, Yoneda lemma)
\end{itemize}

This work is formalized in type theory and intuitionistic logic. Category theory appears later in the development to analyze
the structures that emerge from the main types defined in the theory. This work aims to be fully constructive wherever possible,
...
This definition immediately leads to paradoxes. If \textit{every} proposition has a witness, then contradictory propositions must also have witnesses. Intentionally introducing a paradoxical definition might seem rather unorthodox, but analyzing the extremes that break logic will inform us about how to introduce consistency later on. Let's follow this thread and see where it takes us.
...
Notice that OmegaType is a class that wraps a carrier type along with specifying its \textit{omega completeness} property. By encapsuling the carrier type this way, we can treat it as a hypothetical and reason about it with a layer of abstraction. This allows us to explore its paradoxical properties without collapsing the entire type theory into triviality. An instance of OmegaType need not exist (i.e. OmegaType need not be inhabited), but we can still reason about propositions over OmegaType. Similar to how category theory does not analyze the internal contents of objects but instead focuses on morphisms between objects, in this theory, we will focus on logical propositions on types but rarely focus on the actual content of the inhabitants.
