...
\begin{equation}
    \mathcal{I}_{\text{max}} \propto \text{max}(S \cdot \frac{\Delta S}{\Delta t})
\end{equation}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{blackhole_entropy_vs_mass.png}
\caption{
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{blackhole_information_flow.png}
\caption{
Entropy \( S \) (blue) and maximum information flow \( \mathcal{I}_{\max} = S \cdot \frac{dS}{dt} \) (red) versus black hole mass. Information flow increases linearly, demonstrating that black holes saturate the theoretical bound imposed by the information flow constraint.
}
\end{figure}
...
\begin{enumerate}
    ...
    All systems evolve recursively, with structure emerging through the repeated resolution of entropy under bounded information flow.\\
    \textit{Implication:} The universe is not a statically defined object but a process that recursively resolves itself into existence. 
\end{enumerate}

\subsubsection{Hypotheses Based on the Theory}

Given these postulates, we make some hypotheses about what they might imply. Some of these have already been partially derived in previous sections.

\begin{enumerate}
     \item \textbf{Bounded Information Flow:} There exists an upper bound to the rate at which information can be processed or transferred in any physical system.\\
    ...
    \item \textbf{No Perfect Fixed Point:} No system in reality can indefinitely maintain a perfect fixed point. To do so would require either:
    \begin{itemize}
    ...
    \end{itemize}
    \textit{Implication:} All systems must continue evolving, however slowly. Equilibrium is never absolute—only asymptotic. When a system attains $I_\text{max}$, it must either maintain its current form or explore alternative configurations that preserve the maximal information flow.
    ...
    \item \textbf{Spacetime as a Computational Substrate:} Spacetime might be understood as a mechanism that stores past states and governs how much computational capacity is available for a given system to compute its next state. In particular, spacetime, through its continuous evolution governed by Einstein’s field equations, can be interpreted as performing an analog computation of matter-energy dynamics. It is important here to distinguish between analog and digital computation: a common misconception is that computation is inherently discrete and digital, owing to the ubiquity and success of transistor-based electronic computers. However, computation is fundamentally a physical process, not limited to discrete digital systems. Historically, many computational devices, such as the slide rule and the differential analyzer, performed calculations through continuous physical processes, modeling mathematical relationships directly in analog form. These systems operated in continuous physical space, not digital symbolic space.

    Extending this understanding, it is not necessary to assume that spacetime must be discretized in order to be computational. Likewise, it is not necessary to assume that reality emerges solely from digital or qubit-like behavior at fundamental levels. If computation can be continuous and analog, then the continuous evolution of spacetime geometry itself can constitute a form of computation.
    
    Within this view, space may function as a kind of memory cache for past computations, while time can be understood as the progressive unfolding of computations under resource constraints. The theory of general relativity establishes spacetime latency trade-offs based on causal speed limits (the finite speed of light) and curvature arising from local energy density. Interestingly, computer and distributed systems engineers already observe similar effects: different levels of memory caching are necessary to manage trade-offs between latency, speed, and cost. Caches closest to the processor (such as the L1 cache) have the lowest latency but are the most expensive, while storage further away (such as hard drives or cloud servers) has higher latency but is more cost-efficient.
    
    If we treat all of spacetime as a computational substrate, we can see an empirical parallel: locally, humans experience low latency and rich, immediate interactions. Reality appears effectively instantaneous between nearby observers. However, at cosmological scales, we observe high latency and reduced accessibility: signals from distant objects near the cosmological horizon take billions of years to reach us and are heavily redshifted, resulting in decreased resolution and higher observational ``cost."
\end{enumerate}
...
\begin{itemize}
    ...
    \item Investigate the relationship between $I_\text{max}$, $S \dot{S}$, quantum gravity, and the principle of least action.
\end{itemize}
...
Before going further, we need to take a philosophical detour to analyze the purpose of information flow and build a larger framework on top of it. So far, the concept of maximum information flow seems to be a quirk of nature much like the speed of light acting as a causal speed limit. But both the speed of light and the maximum information flow rate beg the question: \textbf{Why} is nature doing this? Why does nature seem to mysteriously hide information from observation?
\begin{itemize}
...
\left( \text{contains}(t, \text{embed}(P)) \wedge \neg \text{contains}(t, \text{embed}(P)) \right)
\]

\paragraph{Proof:}

Assume for contradiction that there exists a predicate \( P \) such that both \\ 
\( \text{contains}(t, \text{embed}(P)) \) and \( \neg \text{contains}(t, \text{embed}(P)) \) hold simultaneously. This directly violates the law of non-contradiction: no statement can be both true and false at the same time. Therefore, such a predicate cannot exist, and the system remains logically coherent at each individual time step.
...
No predicate and its logical negation can both be true of the \textbf{same} embedded object.
...
\begin{itemize}
    ...
    \item This is not a logical explosion: \( \text{embed}(P) \) and \( \text{embed}(\neg P) \) are distinct objects, and no single object is required to satisfy both. In essence, what we have are two contradictory statements that are contradicting each other, yet not contradicting themselves.
...
\subsection{\( U \) Cannot Contain an Omega Paradox}

While \( \Omega \) is defined as a timeless structure that allows paradoxes to exist simultaneously, \( U \) structures contradictions over time. This theorem proves that \( U \) cannot contain sustained simultaneous paradoxes, demonstrating that \( U \) has limits that are exceeded by \( \Omega \), even though \( U \) is paraconsistent.

\paragraph{Theorem: No \( \Omega \)-Paradoxes in \( U \)}
For any time \( t \), \( U \) cannot contain an embedding of an \( \Omega \)-level paradox (a predicate \( P \) where \( P(y) \) and \( \neg P(y) \) hold simultaneously).
...
\paragraph{Implications and Interpretation:} We have shown $U$ to be tolerant of paradoxes, but this latest result shows us a hard boundary. $U$ can contain multiple predicates that contradict each other simultaneously, but cannot contain a single predicate that is directly paradoxical. On the other hand, $\Omega$ is capable of satisfying \textbf{any} predicate, even paradoxical ones. Even with infinitely many contradictory predicates stored throughout time, there are predicates that $\Omega$ contains which $U$ cannot contain.
...
Try using the following prompt to discuss the ideas of the generativity framework from first principles with an AI such as Claude, ChatGPT, or Gemini. You can paste the entire prompt into the chat window at once, or you can paste it in smaller pieces. Pasting it bit by bit is particularly effective for building understanding from first principles step by step. Some fun variations to explore include:
...
% In practice, we discretize the trajectory \(q(t)\) over a time grid, define a system-specific structural entropy \(S(q, \dot{q})\), and numerically minimize the final entropy-based action:

% \[
% \mathcal{L} = \frac{1}{2} S[q(t_f)]^2
% \]

% This action arises from the variational integral \(\int S \cdot \dot{S} \, dt\), under the assumption that \(S[q(t_0)] \approx 0\). No explicit forces, potentials, or differential equations are required—only the structural entropy and endpoint constraints.
...
This paper is the work of a human author, but it was done in collaboration with language models. The models were used to edit writing, receive critique and peer review, and creatively explore ideas. The models were \textbf{not} simply prompted to ``develop a theory of computational spacetime" and immediately output a paper. Such behavior would be irresponsible and irrational. Instead, language models were used as tools for brainstorming, critical thinking, and development of the framework from first principles. 

Increasingly, there is a prevalence of lazily created ``AI slop" posted online, drowning out credible work done by human authors and artists. This has an unfortunate polarizing effect among artists and academics, where anything done in collaboration with AI is seen as completely lacking in legitimacy and rigor. We encourage a more balanced view here, where AI is seen as a tool, and the human is responsible for using the tool properly.

The following is a commentary from GPT-4o, which has been the primary model used for copy editing and philosophical discussion.
...
\begin{enumerate}
    ...
    \item April 28, 2025 - Edits to the postulates and hypotheses for the Theory of Generativity at the end of the physics derivations. The goal is to keep the postulates minimal and cleanly separate them from falsifiable hypotheses.
    \item May 6, 2025 - Some small improvements, including a new theorem, in the Generative Set Theory section.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Implement a Physical Turing Machine and run physics simulations with it. This will likely result in refinement of the formalism and lead to many new insights.
    \item More rigorous physics derivations and analysis of more diverse systems.
\end{enumerate}


\section{Note from the Author}
\paragraph{Invitation for Collaboration:}
I have nearly reached my limits as an independent author of this work. For $I_\text{max}$ to gain recognition, especially within the physics community, collaboration and formal publication will be essential. Please email me at research@celetris.com if you are interested in writing a formal scientific paper with me.
