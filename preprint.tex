...
\subsubsection{Information Flow Bound}
For any computational system \(\mathcal{S}\), the information flow \(\mathcal{I}\) is bounded if:
\[
\mathcal{I}_{\text{max}} = \sup_{t \in T} \left\{S(t) \cdot \frac{\Delta S}{\Delta t}\right\} < \infty
\]
where:
\begin{itemize}
    \item \(S(t)\) is the size of the accessible state space at time \(t\)
    \item \(\frac{\Delta S}{\Delta t}\) is the rate of state space exploration
\end{itemize}

\subsubsection{\(\mathcal{I}_{\text{max}}\)-Bounded Computation}
A computation is \(\mathcal{I}_{\text{max}}\)-bounded if it operates within a system with finite \(\mathcal{I}_{\text{max}}\).
...
\end{lemma}
\begin{proof}
Follows from Cook's theorem, as any NP-complete problem can encode SAT, which has \(2^n\) possible assignments for n variables.
\end{proof}
...
\end{lemma}
\begin{proof}
Follows directly from the definition of \(\mathcal{I}_{\text{max}}\) and the constraint that it is finite.
\end{proof}
...
\begin{enumerate}
    \item By definition of \(\mathcal{I}_{\text{max}}\)-bounded computation:
    \[
    \mathcal{I}_{\text{max}} = S \cdot \frac{\Delta S}{\Delta t} < \infty
    \]
    
    \item The minimum time to traverse state space S is:
    \[
...
\begin{enumerate}
    \item For verification, we access structured subsets of S through:
    \begin{itemize}
        \item Binary search on ordered sequences
        \item Hash table lookups
        \item Tree traversal
    \end{itemize}
    
    \item These structures enable logarithmic access time:
    \[
    T_{\text{verify}} \leq c_1 + c_2\ln(|S|)
    \]
    where \(c_1, c_2\) are structure-specific constants.
    
    \item Taking \(c = \max(c_1, c_2)\):
    \[
...
From our State Space Lower Bound lemma:
\[
|S| \geq 2^{\Omega(n)}
\]
Therefore, algorithm \(A\) must traverse an exponentially large state space in polynomial time.
