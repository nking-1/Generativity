...
pTermPart = choice
  ...
  , pStr    -- NEW
  , pInt
  , pVar    -- NOTE: Dot access handled via expression builder? No, explicit
  ]
...
pStr :: Parser Term
pStr = StrVal <$> lexeme stringLiteral

pVar :: Parser Term
pVar = do
    root <- identifier
    -- Check for dot access: x.y.z
    fields <- many (char '.' >> identifier)
    let base = Var root
    return $ foldl (flip Get) base fields
